...
#define OS_SUPPORT_INTSTACK_INFO           (1)                 // Option to supply more info to embOSView
#define OS_TEXT_SECTION_ATTRIBUTE(name)

#if (defined(__ARM_ARCH_7A__) || defined(__ARM_ARCH_7R__))  // ARMv7
  #define OS_INT_Disable()     { __asm volatile("cpsid i"  : : : "memory"); }
  #define OS_INT_Enable()      { __asm volatile("cpsie i"  : : : "memory"); }
  #define OS_INT_DisableAll()  { __asm volatile("cpsid if" : : : "memory"); }
  #define OS_INT_EnableAll()   { __asm volatile("cpsie if" : : : "memory"); }
#elif (defined(__thumb__))                                  // ARMv4/5/6 thumb mode
  #define OS_INT_Disable()     OS_INT_DisableFunc()
  #define OS_INT_Enable()      OS_INT_EnableFunc()
#else                                                       // ARMv4/5/6 arm mode
  #define OS_INT_Disable()     { __asm volatile("mrs  R12, CPSR       \n\t" \
                                                "orr  R12, R12, #0x80 \n\t" \
                                                "msr  CPSR_c, R12     \n\t" \
                                                :                           \
                                                :                           \
  ...
  #define OS_INT_Enable()      { __asm volatile("mrs  R12, CPSR       \n\t" \
                                                "bic  R12, R12, #0x80 \n\t" \
                                                "msr  CPSR_c, R12     \n\t" \
                                                :                           \
                                                :                           \
...
*
*  Define Floating-point/SIMD specific macros using predefined macros
*  according to the Arm C Language Extensions (ACLE).
*  It is assumed that if the target includes an FPU without NEON,
*  only 16 double-precision registers are supported by the target.
*  An exception to this is the Cortex-A15 which can include a VFPv4
*  with 32 double-precision registers, but no NEON. In this case
*  OS_VFP_D32 has to be defined .e.g. in the project settings.
*  If the target includes a NEON, always 32 double-precision registers
*  are supported by the target.
*/
#if (!defined(OS_VFP_D16) && !defined(OS_VFP_D32))
...
typedef struct {
#if (defined(OS_VFP_D16) || defined(OS_VFP_D32))
  OS_U32 OS_REG_FPSCR;  // Pushed/popped together with R4-R11 and LR
  OS_U32 Dummy;         // Insert padding word explicitly
#endif
  OS_U32 Counters;
  OS_U32 OS_REG_R4;
  OS_U32 OS_REG_R5;
...
void            OS_ARM_AssignISRSource  (int ISRIndex, int Source)                  OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_AssignISRSource);
void            OS_ARM_ClearPendingFlag (int ISRIndex)                              OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_ClearPendingFlag);
void            OS_ARM_DisableISR       (int ISRIndex)                              OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_DisableISR);
void            OS_ARM_DisableISRSource (int SourceIndex)                           OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_DisableISRSource);
void            OS_ARM_EnableISR        (int ISRIndex)                              OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_EnableISR);
void            OS_ARM_EnableISRSource  (int SourceIndex)                           OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_EnableISRSource);
...
unsigned int    OS_ARM_IsPending        (int ISRIndex)                              OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_IsPending);
int             OS_ARM_ISRSetPrio       (int ISRIndex, int Prio)                    OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_ISRSetPrio);
int             OS_ARM_ISRSetSecure     (int ISRIndex)                              OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_ISRSetSecure);
int             OS_ARM_ISRSetUnSecure   (int ISRIndex)                              OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_ISRSetUnSecure);
void            OS_ARM_SetISRCfg        (int ISRIndex, OS_U32 Cfg)                  OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_SetISRCfg);
void            OS_irq_handler          (void)                                      OS_TEXT_SECTION_ATTRIBUTE(OS_irq_handler);      // C-level interrupt handler in RTOSInit
void            OS_USER_irq_func        (void)                                      OS_TEXT_SECTION_ATTRIBUTE(OS_USER_irq_func);    // User interrupt handler from UserIrq.c

#if (defined(__thumb__) && !defined(__ARM_ARCH_7A__) && !defined(__ARM_ARCH_7R__))  // ARMv4/5/6 thumb mode
void            OS_INT_DisableFunc      (void)                                      OS_TEXT_SECTION_ATTRIBUTE(OS_INT_DisableFunc);  // ARM mode function
void            OS_INT_EnableFunc       (void)                                      OS_TEXT_SECTION_ATTRIBUTE(OS_INT_EnableFunc);   // ARM mode function
#endif
...
#define OS_ExtendTaskContext_TLS_VFP       OS_TLS_SetTaskContextExtension
#define OS_TASK_SetContextExtensionTLSVFP  OS_TLS_SetTaskContextExtension

/*********************************************************************
*
*       Support for ARM VFP/NEON
*
*  Macros and functions are not longer necessary but will be kept for
*  compatibility because VFP/NEON registers are now automatically
*  preserved by embOS.
*/
extern const struct OS_EXTEND_TASK_CONTEXT_STRUCT OS_Dummy_ContextExtension;

#define OS_NEON_ContextExtension    OS_Dummy_ContextExtension
#define OS_VFP_ExtendContext        OS_Dummy_ContextExtension
#define OS_VFPD16_ContextExtension  OS_Dummy_ContextExtension
#define OS_VFPD32_ContextExtension  OS_Dummy_ContextExtension
#define OS_NEON_ExtendContext       OS_Dummy_ContextExtension
#define OS_VFPD16_SetTaskContextExtension()
#define OS_VFPD32_SetTaskContextExtension()
#define OS_NEON_SetTaskContextExtension()
#define OS_ExtendTaskContext_VFP()
#define OS_ExtendTaskContext_NEON()
#define OS_TASK_SetContextExtensionVFP()
#define OS_TASK_SetContextExtensionNEON()
#define OS_VFP_Save()
#define OS_VFP_Restore()
#define OS_NEON_Save()
#define OS_NEON_Restore()
#define OS_VFPD16_Save()
#define OS_VFPD16_Restore()
#define OS_VFPD32_Save()
#define OS_VFPD32_Restore()

/*********************************************************************
*
*       Support for ARM MMU/MPU and Caches
*/
#if (defined (__ARM_ARCH_7A__) && (__ARM_ARCH_7A__ == 1))
  #define OS_ARM_AddL2Cache                      OS_ARM_v7_AddL2Cache
  #define OS_ARM_CACHE_GetLineSize               OS_ARM_v7_CACHE_GetLineSize
  #define OS_ARM_CACHE_Sync                      OS_ARM_v7_CACHE_Sync
  #define OS_ARM_DCACHE_Clean                    OS_ARM_v7_DCACHE_Clean
  #define OS_ARM_DCACHE_CleanRange               OS_ARM_v7_DCACHE_CleanRange
  #define OS_ARM_DCACHE_Enable                   OS_ARM_v7_DCACHE_Enable
  #define OS_ARM_DCACHE_Invalidate               OS_ARM_v7_DCACHE_Invalidate
  #define OS_ARM_DCACHE_InvalidateRange          OS_ARM_v7_DCACHE_InvalidateRange
  #define OS_ARM_ICACHE_Enable                   OS_ARM_v7_ICACHE_Enable
  #define OS_ARM_ICACHE_Invalidate               OS_ARM_v7_ICACHE_Invalidate
  #define OS_ARM_MMU_AddTTEntries                OS_ARM_v7_MMU_AddTTEntries
  #define OS_ARM_MMU_Enable                      OS_ARM_v7_MMU_Enable
  #define OS_ARM_MMU_GetVirtualAddr              OS_ARM_v7_MMU_GetVirtualAddr
  #define OS_ARM_MMU_InitTT                      OS_ARM_v7_MMU_InitTT
  #define OS_ARM_MMU_IsEnabled                   OS_ARM_v7_MMU_IsEnabled
  #define OS_ARM_MMU_v2p                         OS_ARM_v7_MMU_v2p
  #define OS_ARM_SetVBAR                         OS_ARM_v7_SetVBAR

  // According to DDI0406B A3.5.8 and B3.7.2, cache modes are defined as follows
  // TEX[2..0] => b14..b12, C => b3, B => b2 of descriptor
  //                                             TEX[14:12]      C[3]        B[2]
  #define OS_ARM_CACHEMODE_NC_NB                 ((0u   << 12) | (0u << 3) | (0u << 2))  // deprecated, for compatibility with older embOS versions only
  #define OS_ARM_CACHEMODE_C_NB                  ((0u   << 12) | (1u << 3) | (0u << 2))  // deprecated, for compatibility with older embOS versions only
  #define OS_ARM_CACHEMODE_NC_B                  ((0u   << 12) | (0u << 3) | (1u << 2))  // deprecated, for compatibility with older embOS versions only
  #define OS_ARM_CACHEMODE_C_B                   ((1u   << 12) | (1u << 3) | (1u << 2))  // deprecated, for compatibility with older embOS versions only
  #define OS_ARM_CACHEMODE_STRONGLY_ORDERED      ((0u   << 12) | (0u << 3) | (0u << 2))  // Strongly ordered
  #define OS_ARM_CACHEMODE_SHAREABLE_DEVICE      ((0u   << 12) | (0u << 3) | (1u << 2))  // Shareable Device
  #define OS_ARM_CACHEMODE_WRITE_THROUGH         ((0u   << 12) | (1u << 3) | (0u << 2))  // Outer and Inner Write-Through, no Write-Allocate
  #define OS_ARM_CACHEMODE_WRITE_BACK_NO_ALLOC   ((0u   << 12) | (1u << 3) | (1u << 2))  // Outer and Inner Write-Back, no Write-Allocate
  #define OS_ARM_CACHEMODE_NON_CACHEABLE         ((1u   << 12) | (0u << 3) | (0u << 2))  // Outer and Inner Non-cacheable
  #define OS_ARM_CACHEMODE_WRITE_BACK_ALLOC      ((1u   << 12) | (1u << 3) | (1u << 2))  // Outer and Inner Write-Back, Write-Allocate
  #define OS_ARM_CACHEMODE_NON_SHAREABLE_DEVICE  ((2u   << 12) | (0u << 3) | (0u << 2))  // Non-shareable Device
  #define OS_ARM_CACHEMODE_ANY                   ((0x7u << 12) | (0x0Fu))
  #define OS_ARM_MMU_EXECUTE_NEVER               (1u <<  4)                              // Determines whether the region is executable
  #define OS_ARM_MMU_READWRITE                   (0u << 10)                              // Full access
  #define OS_ARM_MMU_NOACCESS                    (1u << 10)                              // All accesses generate Permission faults
  #define OS_ARM_MMU_READONLY                    (2u << 10)                              // Privileged and User read-only
  #define OS_ARM_MMU_SHAREABLE                   (1u << 16)                              // Shareable attribute
  ...
  //                                             TEX[5:3]     C[1]        B[0]
  #define OS_ARM_CACHEMODE_STRONGLY_ORDERED      ((0u << 3) | (0u << 1) | (0u << 0))     // Strongly ordered
  #define OS_ARM_CACHEMODE_SHAREABLE_DEVICE      ((0u << 3) | (0u << 1) | (1u << 0))     // Shareable Device
  #define OS_ARM_CACHEMODE_WRITE_THROUGH         ((0u << 3) | (1u << 1) | (0u << 0))     // Outer and Inner Write-Through, no Write-Allocate
  #define OS_ARM_CACHEMODE_WRITE_BACK_NO_ALLOC   ((0u << 3) | (1u << 1) | (1u << 0))     // Outer and Inner Write-Back, no Write-Allocate
  #define OS_ARM_CACHEMODE_NON_CACHEABLE         ((1u << 3) | (0u << 1) | (0u << 0))     // Outer and Inner Non-cacheable
  #define OS_ARM_CACHEMODE_WRITE_BACK_ALLOC      ((1u << 3) | (1u << 1) | (1u << 0))     // Outer and Inner Write-Back, Write-Allocate
  #define OS_ARM_CACHEMODE_NON_SHAREABLE_DEVICE  ((2u << 3) | (0u << 1) | (0u << 0))     // Non-shareable Device

  #define OS_ARM_MPU_NOACCESS                    (0u <<  8)
  #define OS_ARM_MPU_READWRITE                   (3u <<  8)
  #define OS_ARM_MPU_READONLY                    (6u <<  8)
  #define OS_ARM_MPU_EXECUTE_NEVER               (1u << 12)
  #define OS_ARM_MPU_SHAREABLE                   (1u <<  2)
#elif ((defined (__ARM_ARCH_5__)    && (__ARM_ARCH_5__    == 1)) || \
       (defined (__ARM_ARCH_5E__)   && (__ARM_ARCH_5E__   == 1)) || \
       (defined (__ARM_ARCH_5T__)   && (__ARM_ARCH_5T__   == 1)) || \
       (defined (__ARM_ARCH_5TE__)  && (__ARM_ARCH_5TE__  == 1)) || \
       (defined (__ARM_ARCH_5TEJ__) && (__ARM_ARCH_5TEJ__ == 1)))
  #define OS_ARM_CACHE_Sync                      OS_ARM_v5E_CACHE_Sync
  #define OS_ARM_DCACHE_Clean                    OS_ARM_v5E_DCACHE_Clean
  #define OS_ARM_DCACHE_CleanRange               OS_ARM_v5E_DCACHE_CleanRange
  #define OS_ARM_DCACHE_Enable                   OS_ARM_v5E_DCACHE_Enable
  #define OS_ARM_DCACHE_Invalidate               OS_ARM_v5E_DCACHE_Invalidate
  ...
  #define OS_ARM_ICACHE_Invalidate               OS_ARM_v5E_ICACHE_Invalidate
  #define OS_ARM_MMU_AddTTEntries                OS_ARM_v5E_MMU_AddTTEntries
  #define OS_ARM_MMU_Enable                      OS_ARM_v5E_MMU_Enable
  #define OS_ARM_MMU_GetVirtualAddr              OS_ARM_v5E_MMU_GetVirtualAddr
  #define OS_ARM_MMU_InitTT                      OS_ARM_v5E_MMU_InitTT
  #define OS_ARM_MMU_IsEnabled                   OS_ARM_v5E_MMU_IsEnabled
  #define OS_ARM_MMU_v2p                         OS_ARM_v5E_MMU_v2p

  #define OS_ARM_CACHEMODE_NC_NB                 ((0u << 3) | (0u << 2))
  #define OS_ARM_CACHEMODE_C_NB                  ((1u << 3) | (0u << 2))
  #define OS_ARM_CACHEMODE_NC_B                  ((0u << 3) | (1u << 2))
  #define OS_ARM_CACHEMODE_C_B                   ((1u << 3) | (1u << 2))
  #define OS_ARM_CACHEMODE_ANY                   (0x0Fu)
#else  // Dummy defines for compiling ARMv5/ARMv7A code on ARMv4 targets
  #define OS_ARM_AddL2Cache(a,b)
  #define OS_ARM_CACHE_GetLineSize(a)            (0u)
  #define OS_ARM_CACHE_Sync()
  #define OS_ARM_DCACHE_Clean()
  #define OS_ARM_DCACHE_CleanRange(a,b)
  #define OS_ARM_DCACHE_Enable()
  #define OS_ARM_DCACHE_Invalidate()
  #define OS_ARM_DCACHE_InvalidateRange(a,b)
  #define OS_ARM_ICACHE_Enable()
  #define OS_ARM_ICACHE_Invalidate()
  #define OS_ARM_MMU_AddTTEntries(a,b,c,d,e)
  #define OS_ARM_MMU_Enable(a)
  #define OS_ARM_MMU_GetVirtualAddr(a,b)         (0u)
  #define OS_ARM_MMU_InitTT(a)
  #define OS_ARM_MMU_IsEnabled()                 (0u)
  #define OS_ARM_MMU_v2p(a)                      (0u)
  #define OS_ARM_SetVBAR(a)

  #define OS_ARM_CACHEMODE_NC_NB                 (0u)
  #define OS_ARM_CACHEMODE_C_NB                  (0u)
  #define OS_ARM_CACHEMODE_NC_B                  (0u)
  #define OS_ARM_CACHEMODE_C_B                   (0u)
  #define OS_ARM_CACHEMODE_STRONGLY_ORDERED      (0u)
  #define OS_ARM_CACHEMODE_SHAREABLE_DEVICE      (0u)
  #define OS_ARM_CACHEMODE_WRITE_THROUGH         (0u)
  #define OS_ARM_CACHEMODE_WRITE_BACK_NO_ALLOC   (0u)
  #define OS_ARM_CACHEMODE_NON_CACHEABLE         (0u)
  #define OS_ARM_CACHEMODE_WRITE_BACK_ALLOC      (0u)
  #define OS_ARM_CACHEMODE_NON_SHAREABLE_DEVICE  (0u)
  #define OS_ARM_CACHEMODE_ANY                   (0u)
#endif

#define OS_ARM_CACHEMODE_ILLEGAL                 (1u)  // Obsolete, we just keep it for compatibility
#define OS_ARM_MMU_UNMAPPED                      (1u)

#if ((defined (__ARM_ARCH_7A__)   && (__ARM_ARCH_7A__   == 1)) || \
     (defined (__ARM_ARCH_7R__)   && (__ARM_ARCH_7R__   == 1)) || \
     (defined (__ARM_ARCH_5__)    && (__ARM_ARCH_5__    == 1)) || \
     (defined (__ARM_ARCH_5E__)   && (__ARM_ARCH_5E__   == 1)) || \
     (defined (__ARM_ARCH_5T__)   && (__ARM_ARCH_5T__   == 1)) || \
     (defined (__ARM_ARCH_5TE__)  && (__ARM_ARCH_5TE__  == 1)) || \
     (defined (__ARM_ARCH_5TEJ__) && (__ARM_ARCH_5TEJ__ == 1)))
OS_U32 OS_ARM_CACHE_GetLineSize     (OS_U32 CIndex)                                                                                OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_CACHE_GetLineSize);
void   OS_ARM_CACHE_Sync            (void)                                                                                         OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_CACHE_Sync);
void   OS_ARM_DCACHE_Clean          (void)                                                                                         OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_DCACHE_Clean);
void   OS_ARM_DCACHE_CleanRange     (void* p, OS_U32 NumBytes)                                                                     OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_DCACHE_CleanRange);
void   OS_ARM_DCACHE_Enable         (void)                                                                                         OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_DCACHE_Enable);
void   OS_ARM_DCACHE_Invalidate     (void)                                                                                         OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_DCACHE_Invalidate);
void   OS_ARM_DCACHE_InvalidateRange(void* p, OS_U32 NumBytes)                                                                     OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_DCACHE_InvalidateRange);
void   OS_ARM_ICACHE_Enable         (void)                                                                                         OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_ICACHE_Enable);
void   OS_ARM_ICACHE_Invalidate     (void)                                                                                         OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_ICACHE_Invalidate);
void   OS_ARM_MMU_AddTTEntries      (OS_U32* pTranslationTable, OS_U32 CacheMode, OS_U32 VIndex, OS_U32 PIndex, OS_U32 NumEntries) OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_MMU_AddTTEntries);
void   OS_ARM_MMU_Enable            (OS_U32* pTranslationTable)                                                                    OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_MMU_Enable);
void*  OS_ARM_MMU_GetVirtualAddr    (OS_U32 PAddr, OS_U32 CacheMode)                                                               OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_MMU_GetVirtualAddr);
void   OS_ARM_MMU_InitTT            (OS_U32* pTranslationTable)                                                                    OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_MMU_InitTT);
OS_U32 OS_ARM_MMU_IsEnabled         (void)                                                                                         OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_MMU_IsEnabled);
OS_U32 OS_ARM_MMU_v2p               (void* pVAddr)                                                                                 OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_MMU_v2p);
void   OS_ARM_MPU_AddEntry          (OS_U32 Region, void* BaseAddr, OS_U32 Size, OS_U32 Permissions, OS_U32 Attributes)            OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_MPU_AddEntry);
void   OS_ARM_MPU_Enable            (void)                                                                                         OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_MPU_Enable);
OS_U32 OS_ARM_MPU_GetMinRegionSize  (void)                                                                                         OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_MPU_GetMinRegionSize);
OS_U32 OS_ARM_MPU_GetNumRegions     (void)                                                                                         OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_MPU_GetNumRegions);
void   OS_ARM_MPU_Init              (void)                                                                                         OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_MPU_Init);
void   OS_ARM_SetVBAR               (OS_U32 Addr)                                                                                  OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_SetVBAR);
#endif

/*********************************************************************
*
*       Support for ARM L2 Caches
*/
typedef struct {
  void (*pfEnable)         (void* pParam);
  void (*pfSync)           (void* pParam);
  void (*pfInvalidate)     (void* pParam);
  void (*pfInvalidateRange)(void* pParam, void* pAddr, unsigned int NumBytes);
  void (*pfClean)          (void* pParam);
  void (*pfCleanRange)     (void* pParam, void* pAddr, unsigned int NumBytes);
} OS_ARM_L2CACHE_API;

void OS_ARM_v7_AddL2Cache(const OS_ARM_L2CACHE_API* pCacheAPI, void* pParam) OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_v7_AddL2Cache);

extern const OS_ARM_L2CACHE_API OS_L2CACHE_L2C310;
extern const OS_ARM_L2CACHE_API OS_L2CACHE_XilinxZynq7000;

/*********************************************************************
*
*       Support for ARM Generic Interrupt Controller
*/
//
// Private peripheral IDs
//
#define OS_MPCORE_PPI_PRIVATE_TIMER  (29u)

OS_U32 OS_GIC_GetCBAR                (void)                   OS_TEXT_SECTION_ATTRIBUTE(OS_GIC_GetCBAR);
void   OS_GIC_SetBaseAddr            (OS_U32 BaseAddr)        OS_TEXT_SECTION_ATTRIBUTE(OS_GIC_SetBaseAddr);
void   OS_GIC_SetCPUInterfaceBaseAddr(OS_U32 BaseAddr)        OS_TEXT_SECTION_ATTRIBUTE(OS_GIC_SetCPUInterfaceBaseAddr);
void   OS_GIC_SetDistributionBaseAddr(OS_U32 BaseAddr)        OS_TEXT_SECTION_ATTRIBUTE(OS_GIC_SetDistributionBaseAddr);
void   OS_GIC_SetIntSecure           (OS_U32 Id)              OS_TEXT_SECTION_ATTRIBUTE(OS_GIC_SetIntSecure);
void   OS_GIC_SetIntUnsecure         (OS_U32 Id)              OS_TEXT_SECTION_ATTRIBUTE(OS_GIC_SetIntUnsecure);
void   OS_GIC_EnableInt              (OS_U32 Id)              OS_TEXT_SECTION_ATTRIBUTE(OS_GIC_EnableInt);
void   OS_GIC_DisableInt             (OS_U32 Id)              OS_TEXT_SECTION_ATTRIBUTE(OS_GIC_DisableInt);
OS_U32 OS_GIC_IsPending              (OS_U32 Id)              OS_TEXT_SECTION_ATTRIBUTE(OS_GIC_IsPending);
void   OS_GIC_ClrPending             (OS_U32 Id)              OS_TEXT_SECTION_ATTRIBUTE(OS_GIC_ClrPending);
OS_U32 OS_GIC_SetPrio                (OS_U32 Id, OS_U32 Prio) OS_TEXT_SECTION_ATTRIBUTE(OS_GIC_SetPrio);
void   OS_GIC_SetIntCfg              (OS_U32 Id, OS_U32 Cfg)  OS_TEXT_SECTION_ATTRIBUTE(OS_GIC_SetIntCfg);
void   OS_GIC_SetTargetProc          (OS_U32 Id, OS_U32 Cfg)  OS_TEXT_SECTION_ATTRIBUTE(OS_GIC_SetTargetProc);
OS_U32 OS_GIC_AckInt                 (void)                   OS_TEXT_SECTION_ATTRIBUTE(OS_GIC_AckInt);
void   OS_GIC_EOI                    (OS_U32 Id)              OS_TEXT_SECTION_ATTRIBUTE(OS_GIC_EOI);
OS_U32 OS_GIC_GetNumInts             (void)                   OS_TEXT_SECTION_ATTRIBUTE(OS_GIC_GetNumInts);
void   OS_GIC_SetGroup0_FIQ          (void)                   OS_TEXT_SECTION_ATTRIBUTE(OS_GIC_SetGroup0_FIQ);
void   OS_GIC_SetGroup0_IRQ          (void)                   OS_TEXT_SECTION_ATTRIBUTE(OS_GIC_SetGroup0_IRQ);
void   OS_GIC_SetPrioThreshold       (OS_U32 Threshold)       OS_TEXT_SECTION_ATTRIBUTE(OS_GIC_SetPrioThreshold);
void   OS_GIC_SetPrioGroup           (OS_U32 PrioGroup)       OS_TEXT_SECTION_ATTRIBUTE(OS_GIC_SetPrioGroup);
void   OS_GIC_SetCPUInterfaceCtrl    (OS_U32 r)               OS_TEXT_SECTION_ATTRIBUTE(OS_GIC_SetCPUInterfaceCtrl);
void   OS_GIC_SetDistributorCtrl     (OS_U32 r)               OS_TEXT_SECTION_ATTRIBUTE(OS_GIC_SetDistributorCtrl);
void   OS_GIC_Enable                 (void)                   OS_TEXT_SECTION_ATTRIBUTE(OS_GIC_Enable);
void   OS_GIC_Disable                (void)                   OS_TEXT_SECTION_ATTRIBUTE(OS_GIC_Disable);

/*********************************************************************
*
*       Support for ARM Generic Timer
*/
OS_U64 OS_ARM_GT_GetCompareValue  (void)            OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_GT_GetCompareValue);
OS_U64 OS_ARM_GT_GetCount         (void)            OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_GT_GetCount);
OS_U32 OS_ARM_GT_GetFrequency     (void)            OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_GT_GetFrequency);
OS_U32 OS_ARM_GT_GetISRPending    (void)            OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_GT_GetISRPending);
void   OS_ARM_GT_SetCNTControlBase(OS_U32 BaseAddr) OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_GT_SetCNTControlBase);
void   OS_ARM_GT_SetCompareValue  (OS_U64 Value)    OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_GT_SetCompareValue);
void   OS_ARM_GT_Start            (OS_U32 Period)   OS_TEXT_SECTION_ATTRIBUTE(OS_ARM_GT_Start);
...
*/
#if   (defined(OS_VFP_D16))
  #define OS_Init  OS_Init_VFPD16
#elif (defined(OS_VFP_D32))
  #define OS_Init  OS_Init_VFPD32
#endif
