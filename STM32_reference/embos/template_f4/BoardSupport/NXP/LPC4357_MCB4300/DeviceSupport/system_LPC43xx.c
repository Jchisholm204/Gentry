...
#define PLL0AUDIO_CLK_SEL CLK_SRC_XTAL    /* PLL0USB input clock: XTAL        */
...
/* PLL0AUDIO output clock: 480MHz, Fcco: 480MHz, N = 1, M = 20, P = 1         */
#define PLL0AUDIO_N       1       /* Range [1 -  256]: Pre-divider            */
#define PLL0AUDIO_M      20       /* Range [1 - 2^15]: Feedback-divider       */
#define PLL0AUDIO_P       1       /* Range [1 -   32]: Post-divider           */

#define PLL0AUDIO_DIRECTI 1       /* 0: Use N_DIV, 1: Don't use N_DIV         */
#define PLL0AUDIO_DIRECTO 1       /* 0: Use P_DIV, 1: Don't use P_DIV         */
#define PLL0AUDIO_BYPASS  0       /* 0: Use PLL, 1: PLL is bypassed           */
...
/* PLL0AUDIO Setting Check */
#if (PLL0AUDIO_BYPASS == 0)
 #if (PLL0AUDIO_CLK_SEL == CLK_SRC_XTAL)
  #define PLL0AUDIO_CLKIN CLK_XTAL
 #else
  #define PLL0AUDIO_CLKIN CLK_IRC
 #endif

 #if   ((PLL0AUDIO_DIRECTI == 1) && (PLL0AUDIO_DIRECTO == 1)) /* Mode 1a          */
  #define PLL0AUDIO_FOUT (PLL0AUDIO_CLKIN * 2 * PLL0AUDIO_M)
  #define PLL0AUDIO_FCCO (PLL0AUDIO_FOUT)
 #elif ((PLL0AUDIO_DIRECTI == 1) && (PLL0AUDIO_DIRECTO == 0)) /* Mode 1b          */
  #define PLL0AUDIO_FOUT (PLL0AUDIO_CLKIN * PLL0AUDIO_M / PLL0AUDIO_P)
  #define PLL0AUDIO_FCCO (PLL0AUDIO_FOUT * 2 * PLL0AUDIO_P)
 #elif ((PLL0AUDIO_DIRECTI == 0) && (PLL0AUDIO_DIRECTO == 1)) /* Mode 1c          */
  #define PLL0AUDIO_FOUT (PLL0AUDIO_CLKIN * 2 * PLL0AUDIO_M / PLL0AUDIO_N)
  #define PLL0AUDIO_FCCO (PLL0AUDIO_FOUT)
 #else                                                    /* Mode 1d          */
  #define PLL0AUDIO_FOUT (PLL0AUDIO_CLKIN * PLL0AUDIO_M / (PLL0AUDIO_P * PLL0AUDIO_N))
  #define PLL0AUDIO_FCCO (PLL0AUDIO_FOUT * 2 * PLL0AUDIO_P)
 #endif

 #if (PLL0AUDIO_FCCO < 275000000UL || PLL0AUDIO_FCCO > 550000000UL)
  #error "PLL0AUDIO Fcco frequency out of range! (275MHz >= Fcco <= 550MHz)"
 #endif
 #if (PLL0AUDIO_FOUT < 4300000UL || PLL0AUDIO_FOUT > 550000000UL)
  #error "PLL0AUDIO output frequency out of range! (4.3MHz >= Fclkout <= 550MHz)"
 #endif
#endif
...
#if (defined USE_SPIFI) && (USE_SPIFI)
 ...
 *----------------------------------------------------------------------------*/
  ...
  switch (s) {
    ...
    case 2:  x = 0x00010003;
      break;
    default:
      for (i = PLL0AUDIO_M; i <= 0x8000; i++) {
        x = (((x ^ (x >> 1)) & 1) << 14) | ((x >> 1) & 0x3FFF);
      }
  }

  if (PLL0AUDIO_M < 60) selp = (PLL0AUDIO_M >> 1) + 1;
  else        selp = 31;

  if      (PLL0AUDIO_M > 16384) seli = 1;
  else if (PLL0AUDIO_M >  8192) seli = 2;
  else if (PLL0AUDIO_M >  2048) seli = 4;
  else if (PLL0AUDIO_M >=  501) seli = 8;
  else if (PLL0AUDIO_M >=   60) seli = 4 * (1024 / (PLL0AUDIO_M + 9));
  else                        seli = (PLL0AUDIO_M & 0x3C) + 4;
  LPC_CGU->PLL0AUDIO_MDIV   =  (selp   << 17) |
                             (seli   << 22) |
                             (x      <<  0);
  ...
  switch (s) {
    ...
    case 2:  x = 0x00000202;
      break;
    default:
      for (i = PLL0AUDIO_N; i <= 0x0100; i++) {
        x =(((x ^ (x >> 2) ^ (x >> 3) ^ (x >> 4)) & 1) << 7) | ((x >> 1) & 0x7F);
      }
  }
  LPC_CGU->PLL0AUDIO_NP_DIV = (x << 12);
  ...
  switch (s) {
    ...
    case 2:  x = 0x00000042;
      break;
    default:
      for (i = PLL0AUDIO_P; i <= 0x200; i++) {
        x = (((x ^ (x >> 2)) & 1) << 4) | ((x >> 1) &0x0F);
      }
  }
  LPC_CGU->PLL0AUDIO_NP_DIV |= x;

  LPC_CGU->PLL0AUDIO_CTRL  = (PLL0AUDIO_CLK_SEL   << 24)  | /* Clock source sel                           */
                           (1                 << 13)      | /* MDEC enabled. Fractional divider not used. */
                           (1                 << 11)      | /* Autoblock En                               */
                           (1                 << 4 )      | /* PLL0AUDIO clock en                         */
                           (PLL0AUDIO_DIRECTO   << 3 )    | /* Direct output                              */
                           (PLL0AUDIO_DIRECTI   << 2 )    | /* Direct input                               */
                           (PLL0AUDIO_BYPASS    << 1 )    | /* PLL bypass                                 */
                           (0                 << 0 ) ;      /* PLL0AUDIO Enabled                          */
